...
def main():
    ...
    # 🔍 Ensure the dataset path is set correctly
    args.dataset_path = os.path.abspath(args.dataset_path)  # Convert to absolute path
    print(f"📂 Dataset Path: {args.dataset_path}")  # Debugging

    # 🔹 Explicitly set correct root directory
    correct_root = os.path.abspath(os.path.dirname(__file__))
    print(f"🔍 Using fixed root directory: {correct_root}")  # Debugging
    
    # Load the dataset
    testset = ClassificationData(ModelNet40Data(train=False, root_dir=args.dataset_path))
    ...
    if args.pretrained:
        assert os.path.isfile(args.pretrained), f"❌ Pretrained model not found: {args.pretrained}"
