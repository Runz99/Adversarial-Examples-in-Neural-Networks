...
def load_data(root_dir, train, use_normals):
    ...
    # Look for .h5 files
    h5_files = glob.glob(os.path.join(root_dir, f"modelnet40_{partition}.h5"))
    ...
    if not h5_files:
        print(f"❌ ERROR: No .h5 files found! Check if the dataset exists in {root_dir}.")
        return np.array([]), np.array([])

    for h5_name in h5_files:
        print(f"📂 Loading file: {h5_name}")  # Debugging

        with h5py.File(h5_name, 'r') as f:
            print(f"📂 Available keys: {list(f.keys())}")  # Debugging

            # Check if the required keys exist
            if 'data' not in f or 'label' not in f:
                print(f"⚠️ Skipping {h5_name} (Missing 'data' or 'label')")
                continue
            ...
            print(f"✅ Loaded {data.shape[0]} samples from {h5_name}")  # Debugging
            all_data.append(data)
            all_label.append(labels)

    # Debugging: Print if data was loaded
    if not all_data:
        print("❌ ERROR: No valid data loaded. Check if all .h5 files are empty or incorrectly formatted.")
        return np.array([]), np.array([])

    all_data = np.concatenate(all_data, axis=0)
    all_label = np.concatenate(all_label, axis=0)

    # 🛠 **Fix: Remap Labels to Ensure Continuous Range**
    actual_labels = np.unique(all_label)  # Unique labels in dataset
    label_mapping = {old_label: new_label for new_label, old_label in enumerate(sorted(actual_labels))}
    
    print(f"🔄 Label Mapping: {label_mapping}")  # Debugging

    # Apply the mapping
    all_label = np.vectorize(label_mapping.get)(all_label)

    print(f"✅ Successfully loaded {all_data.shape[0]} samples with shape {all_data.shape}.")
    print(f"📢 Unique labels after remapping: {np.unique(all_label)}")  # Debugging
...
class ModelNet40Data(Dataset):
	def __init__(
        ...
        train=True,
        num_points=1024,
        download=False,  # Keep the argument but prevent unintended download
        root_dir='./',
		...
		print(f"📂 Dataset loading from: {self.root_dir}")

		# 🔹 Only download if explicitly set to True
		if download:
			print("🔽 Downloading ModelNet40 dataset... (This should not be running!)")
			download_modelnet40(root_dir=root_dir)

		print(f"📢 ModelNet40Data initialized! Root dir: {root_dir}, Train: {train}")
		self.data, self.labels = load_data(root_dir, train, use_normals)
		print(f"📢 Unique labels in dataset ({'Train' if train else 'Test'}): {np.unique(self.labels)}")

		# ✅ Ensure dataset is actually loaded
		if self.data.size == 0:
			print("❌ ERROR: No data loaded! Check dataset path:", root_dir)

		print(f"✅ ModelNet40Data loaded {self.data.shape[0]} samples")
	...
	def __getitem__(self, idx):
		...
		# Ensure labels start from 0 and do not exceed the number of classes
		if label < 0 or label >= 33:  # Adjust 40 based on your dataset
			print(f"❌ ERROR: Label {label} is out of range. Check dataset!")
			exit(1)
	...
	def read_classes_ModelNet40(self):
		# Read class names from the shape_names.txt file
		shape_names_file = os.path.join(self.root_dir, 'shape_names.txt')
		if not os.path.exists(shape_names_file):
			raise FileNotFoundError(f"Shape names file not found: {shape_names_file}")
		with open(shape_names_file, 'r') as f:
			shape_names = f.read().splitlines()
		return np.array(shape_names)
